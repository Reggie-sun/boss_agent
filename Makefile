...
CDP_HOST ?= 127.0.0.1
CDP_PORT ?= 9222
CDP_PROFILE ?= $(HOME)/.boss-agent/chrome-cdp
CDP_URL := http://$(CDP_HOST):$(CDP_PORT)
CDP_START_URL ?= https://www.zhipin.com/web/geek/chat
CHROME ?= $(shell command -v google-chrome 2>/dev/null || command -v google-chrome-stable 2>/dev/null || command -v chromium 2>/dev/null || command -v chromium-browser 2>/dev/null)
...
cdp:
	...
	@echo "Chrome launched. Verify with: make cdp-check"
