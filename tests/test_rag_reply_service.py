...
def test_salary_question_uses_preset_reply_without_rag(tmp_path):
	...
	rag_adapter = _FakeRagAdapter(
		_FakeRagResult(ok=True, answer="unused", citations=[])
	)
	service = BossRagReplyService(
		store=store,
		rag_adapter=rag_adapter,
		salary_reply="当前薪资和期望薪资按候选人预设回复。",
	)

	draft = service.create_draft_for_message("msg_001")

	assert draft.intent == "salary_or_offer"
	assert draft.draft_text == "当前薪资和期望薪资按候选人预设回复。"
	assert draft.evidence["source"] == "local_policy"
	assert draft.evidence["reason"] == "salary_preset"
	assert draft.send_allowed is False
	assert draft.approval_required is True
	assert rag_adapter.calls == []


def test_salary_question_falls_back_to_agent_handoff_when_preset_missing(tmp_path):
	store = RagReplyStore(tmp_path / "boss-rag.sqlite3")
	store.initialize()
	store.save_conversation(ConversationRecord(conversation_id="conv_001", source="manual_import"))
	store.save_message(
	...
	assert draft.evidence["source"] == "local_policy"
	assert draft.evidence["reason"] == "salary_handoff"
	assert draft.send_allowed is False
	assert draft.approval_required is True
	assert rag_adapter.calls == []
