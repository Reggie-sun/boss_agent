...
def test_agent_tool_description_includes_availability():
	...
	assert "zhipin" in tool.description
...
@patch("server.shutil.which", return_value="boss")
@patch("server.subprocess.run")
def test_run_boss_parses_json_output(mock_run, _mock_which):
	"""正常 JSON 输出应被正确解析。"""
...
@patch("server.shutil.which", return_value="boss")
@patch("server.subprocess.run")
def test_run_boss_handles_invalid_json(mock_run, _mock_which):
	"""非 JSON 输出应返回错误信封。"""
...
@patch("server.shutil.which", return_value="boss")
@patch("server.subprocess.run")
def test_run_boss_handles_empty_output(mock_run, _mock_which):
	"""空输出应返回错误信封。"""
...
@patch("server.shutil.which", return_value="boss")
@patch("server.subprocess.run")
def test_run_boss_passes_args(mock_run, _mock_which):
	"""参数应正确传递给 subprocess。"""
...
@patch("server.subprocess.run")
def test_run_boss_falls_back_to_python_module_when_boss_not_installed(mock_run, _mock_which):
	mock_run.return_value = MagicMock(stdout='{"ok": true}', stderr="")
	_run_boss("status")
	cmd = mock_run.call_args[0][0]
	assert cmd[:4] == [sys.executable, "-c", "from boss_agent_cli.main import cli; cli()", "--json"]
	assert cmd[4:] == ["status"]


@patch("server.shutil.which", return_value="boss")
@patch("server.subprocess.run")
def test_run_boss_timeout(mock_run, _mock_which):
	"""应设置 120 秒超时。"""
	mock_run.return_value = MagicMock(stdout='{"ok": true}', stderr="")
	_run_boss("doctor")
...
@patch("server.shutil.which", return_value="boss")
@patch("server.subprocess.run")
def test_run_boss_detaches_child_stdin_from_mcp_stdio(mock_run, _mock_which):
...
def test_build_args_shortlist_prepare():
	args = _build_args("boss_shortlist_prepare", {
		"security_id": "s1",
		"job_id": "j1",
		"resume": "backend",
		"tone": "积极主动",
...
def test_build_args_shortlist_mark_applied():
	args = _build_args("boss_shortlist_mark_applied", {
		"security_id": "s1",
		"job_id": "j1",
		"resume": "backend",
		"notes": "官网已手动提交",
