...
def test_config_env_override_for_rag_api_key(monkeypatch, tmp_path):
	from boss_agent_cli.config import load_config
	cfg_file = tmp_path / "config.json"
	cfg_file.write_text(json.dumps({"boss_rag_rag_api_key": "file-value"}), encoding="utf-8")
	monkeypatch.setenv("RAG_API_KEY", "env-value")

	cfg = load_config(cfg_file)

	assert cfg["boss_rag_rag_api_key"] == "env-value"


def test_config_env_override_for_rag_auth_api_key_alias(monkeypatch, tmp_path):
	from boss_agent_cli.config import load_config
	cfg_file = tmp_path / "config.json"
	cfg_file.write_text(json.dumps({"boss_rag_rag_api_key": "file-value"}), encoding="utf-8")
	monkeypatch.setenv("RAG_AUTH_API_KEY", "env-auth-value")

	cfg = load_config(cfg_file)

	assert cfg["boss_rag_rag_api_key"] == "env-auth-value"


def test_config_env_override_parses_rag_boolean_and_int(monkeypatch):
	from boss_agent_cli.config import load_config
	monkeypatch.setenv("BOSS_RAG_ALLOW_MESSAGE_READ", "true")
	monkeypatch.setenv("BOSS_RAG_RAG_TIMEOUT_SECONDS", "33")
...
def test_load_config_reads_watcher_env_aliases(monkeypatch, tmp_path):
	...
	monkeypatch.setenv("BOSS_RAG_PROACTIVE_RESUME_ENABLED", "true")
	...
	assert cfg["boss_rag_proactive_resume_enabled"] is True
