...
import { OutreachWorkspace } from "./views/OutreachWorkspace.jsx";
import "./styles.css";
...
const defaultTenantId = "tenant_local";
const defaultUserId = "user_local";
...
function workspaceFromLocation() {
  ...
  if (pathname.includes("/agent/outreach")) return "outreach";
  return "reply";
}
...
export function App() {
  ...
  function selectWorkspace(nextWorkspace) {
    ...
    const nextPath = nextWorkspace === "outreach" ? "/agent/outreach" : "/agent/reply";
    if (globalThis.history?.pushState && globalThis.location?.pathname !== nextPath) {
  ...
  async function handleBossAutoGreet() {
    const query = bossSearchForm.query.trim();
    if (!query || isBossAutoRunning || bossAutomationRiskLocked || !selectedProfileId) return;
  ...
  return (
    <main className="app-shell app-shell--rag">
      ...
      <section className="workspace">
        <nav className="workspace-tabs" aria-label="Agent workspace">
          <button
            type="button"
            className={activeWorkspace === "reply" ? "is-active" : ""}
            onClick={() => selectWorkspace("reply")}
          >
            <span>发送到 Boss</span>
            <em>发送附件简历 PDF / watcher</em>
          </button>
          <button
            type="button"
            className={activeWorkspace === "outreach" ? "is-active" : ""}
            onClick={() => selectWorkspace("outreach")}
          >
            <span>Boss 自动开聊</span>
            <em>Agent 全自动</em>
          </button>
        ...
        {activeWorkspace === "reply" ? (
          <ReplyWorkspace
            ...
            chatTargetsLoading={chatTargetsLoading}
            loadChatTargets={loadChatTargets}
            isLoading={isLoading}
            handleAsk={handleAsk}
            result={result}
            bridgeState={bridgeState}
            sendResumeWithDraft={sendResumeWithDraft}
            setSendResumeWithDraft={setSendResumeWithDraft}
            handleSendToBoss={handleSendToBoss}
            isSendingToBoss={isSendingToBoss}
          />
        ) : (
          <OutreachWorkspace
            applyMode={applyMode}
            setApplyMode={setApplyMode}
            bossSearchForm={bossSearchForm}
            updateBossSearchForm={updateBossSearchForm}
            bossCityOptions={bossCityOptions}
            bossSalaryOptions={bossSalaryOptions}
            bossExperienceOptions={bossExperienceOptions}
            bossEducationOptions={bossEducationOptions}
            bossIndustryOptions={bossIndustryOptions}
            bossScaleOptions={bossScaleOptions}
            bossStageOptions={bossStageOptions}
            bossJobTypeOptions={bossJobTypeOptions}
            bossExperienceSummary={bossExperienceSummary}
            selectedBossExperiences={selectedBossExperiences}
            toggleBossExperience={toggleBossExperience}
            ...
            bossAutomationProgressTotal={bossAutomationProgressTotal}
            bossAutomationResult={bossAutomationResult}
            bossAutomationHasIssue={bossAutomationHasIssue}
            bossAutomationFailedItems={bossAutomationFailedItems}
            normalizedBossJobs={normalizedBossJobs}
            applyForm={applyForm}
