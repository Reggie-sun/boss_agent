...
| 实习经历  | convoloo，机器学习，2025-05 至 2025-09                    |
| 项目经历  | 乳腺癌良 / 恶性预测系统                                      |
...
* 该企业级 RAG 项目面向制造业内部制度文档、SOP、设备资料与业务知识问答场景。
* 项目覆盖文档入库、OCR/解析、结构化切块、Embedding 索引、混合检索、重排序、权限过滤、引用溯源、智能问答与评测回归。
* 项目累计实现 86 个 API 端点、24 个数据模型。
* 项目支撑企业内部私有知识问答、制度查询、SOP 检索与可追溯文档分析。
...
* 当前企业级 RAG 项目覆盖 86 个 API 端点、24 个数据模型和完整检索问答链路，能够证明其工程能力来自实际项目，而不只是专业名称。
...
Enterprise-grade RAG 是一个基于浏览器、支持私有化部署的企业知识系统，用于企业文档检索、基于证据的智能问答、制度查询、SOP 检索和可追溯文档分析。

该项目面向制造业内部制度文档、SOP、设备资料与业务知识问答场景，由候选人孙瑞杰在宁波伟立机器人科技股份有限公司担任 AI 开发工程师期间独立负责核心架构与落地开发。

项目不是简单的 ChatPDF 或文档聊天 Demo，而是覆盖文档入库、OCR/解析、结构化切块、Embedding 索引、混合检索、重排序、权限过滤、引用溯源、智能问答、多轮追问和评测回归的完整企业级 AI 应用系统。
...
12. 支撑企业内部私有知识问答、制度查询、SOP 检索和可追溯文档分析。
...
| 数据模型                | 24 个                       |
| 检索评测样本              | 167 条                      |
| 当前评测集 Top1 Accuracy | 约 95%+                     |
| 当前评测集 TopK Recall   | 约 97%+                     |
| 支持文档格式              | PDF / Office / PPT 等       |
| 支撑场景                | 私有知识问答、制度查询、SOP 检索、可追溯文档分析 |
...
* 简历中的 167 条检索评测样本、Top1 Accuracy 约 95%+、TopK Recall 约 97%+，应视为当前简历口径。
...
候选人构建 frozen + rolling 双轨评测体系，维护 167 条检索评测样本，覆盖：
...
* 项目累计实现 86 个 API 端点、24 个数据模型，并维护 167 条检索评测样本。
...
| ---------------------------- | --------- |
| 简历当前检索评测样本                   | 167 条     |
| 简历当前 Top1 Accuracy           | 约 95%+    |
| 简历当前 TopK Recall             | 约 97%+    |
...
* 当前维护 167 条检索评测样本，覆盖精确查询、语义查询、多轮追问、跨部门补充召回等场景。
* 当前评测集 Top1 Accuracy 约 95%+，TopK Recall 约 97%+。
...
* 项目累计 86 个 API 端点、24 个数据模型、167 条检索评测样本，体现了工程规模和质量治理意识。
...
我目前在宁波伟立机器人科技股份有限公司担任 AI 开发工程师，主要独立负责一个面向制造业内部制度文档、SOP、设备资料和业务知识问答的企业级 RAG 知识库与智能问答平台。这个项目覆盖文档入库、OCR/解析、结构化切块、Embedding 索引、混合检索、重排序、权限过滤、引用溯源、多轮问答和检索评测回归。项目累计实现 86 个 API 端点、24 个数据模型，维护 167 条检索评测样本，当前评测集 Top1 Accuracy 约 95%+，TopK Recall 约 97%+。

除此之外，我还做过乳腺癌良恶性预测系统，完整实践了数据清洗、特征工程、模型训练、评估和 Streamlit 可视化部署；也在 convoloo 实习期间做过基于 Stable Diffusion、ControlNet 和 LoRA 的故事插画生成系统。
...
* 项目不是简单 Demo，而是包含 86 个 API 端点、24 个数据模型、167 条检索评测样本的完整工程系统。
...
| Result    | 当前评测集 Top1 Accuracy 约 95%+，TopK Recall 约 97%+，编号类和语义类问题稳定性提升                                                     |
...
4. 在 convoloo 实习中接触生成式 AI、Stable Diffusion、ControlNet、LoRA、FastAPI 和 React 全栈集成。
5. 在宁波伟立机器人科技股份有限公司独立负责企业级 RAG 项目，将 LLM、检索、文档处理、权限、评测和部署整合为完整企业 AI 应用。
...
1. 面向制造业内部制度文档、SOP、设备资料和业务知识问答场景。
2. 支持 PDF / Office / PPT 等多格式文档入库。
3. 覆盖 OCR/解析、结构化切块、Embedding 索引、混合检索、重排序、权限过滤、引用溯源、智能问答与评测回归。
4. 实现 86 个 API 端点。
5. 实现 24 个数据模型。
6. 维护 167 条检索评测样本。
7. 当前评测集 Top1 Accuracy 约 95%+。
8. 当前评测集 TopK Recall 约 97%+。
9. 技术栈包括 Python、FastAPI、React、TypeScript、Celery、Redis、Qdrant、PostgreSQL、Elasticsearch、MinIO、Docker、vLLM、BGE-M3、BGE-reranker-v2-m3。
...
5. 企业级 RAG 项目面向制造业内部制度文档、SOP、设备资料和业务知识问答场景。
6. 项目覆盖文档入库、OCR/解析、结构化切块、Embedding 索引、混合检索、重排序、权限过滤、引用溯源、智能问答与评测回归。
7. 项目累计实现 86 个 API 端点、24 个数据模型。
8. 当前维护 167 条检索评测样本。
9. 当前评测集 Top1 Accuracy 约 95%+，TopK Recall 约 97%+。
10. 技术栈包括 FastAPI、React、Qdrant、Elasticsearch、Redis、Celery、PostgreSQL、MinIO、Docker、vLLM、BGE-M3、BGE-reranker-v2-m3。
11. 候选人还做过乳腺癌良 / 恶性预测系统和 convoloo 生成式 AI 实习项目。
12. 候选人熟悉 Codex、Claude Code、MCP、自定义 Skill、Agent Runtime、Harness 和 AGENTS.md 规则体系。

### 16.3 维护说明

1. 如果后续补充正式简历 PDF，应同步更新第 2、3、8、9、10、14 节。
2. 如果企业级 RAG 项目指标变化，应同步更新第 5、6、14 节。
3. 如果新增项目访问地址，应更新第 9、10、14 节。
4. 如果新增工作经历，应更新第 8、13、14 节。
5. 如果候选人确定期望薪资，应更新第 15 节，避免面试 Bot 继续回答“未提供”。
6. 如果项目路线图能力进入生产，应更新第 11.4 节的“已实现 vs 可优化方向”。
...
该项目面向制造业内部制度文档、SOP、设备资料和业务知识问答场景，覆盖文档入库、OCR/解析、结构化切块、Embedding 索引、混合检索、重排序、权限过滤、引用溯源、多轮问答和评测回归，累计实现 86 个 API 端点、24 个数据模型，维护 167 条检索评测样本，当前评测集 Top1 Accuracy 约 95%+、TopK Recall 约 97%+。

候选人还具备机器学习预测系统、生成式 AI 图像应用、LoRA 角色一致性生成、FastAPI 后端、React 前端、Docker 部署和 Agent 工程化经验。整体画像是偏工程落地型的 AI 应用工程师，优势在于能把 LLM、RAG、模型推理、文档处理、权限控制、评测体系和前后端工程整合成可运行、可维护的业务系统。
...
我目前在宁波伟立机器人科技股份有限公司做 AI 开发工程师，主要独立负责一个面向制造业内部制度文档、SOP、设备资料和业务知识问答的企业级 RAG 平台。这个系统不是简单的 ChatPDF，而是覆盖文档入库、OCR/解析、结构化切块、Embedding 索引、Qdrant 向量检索、Elasticsearch BM25、RRF 融合、reranker 重排序、权限过滤、引用溯源、多轮追问和检索评测回归的完整系统。

项目目前实现了 86 个 API 端点、24 个数据模型，维护 167 条检索评测样本，当前评测集 Top1 Accuracy 约 95%+，TopK Recall 约 97%+。我在项目里比较核心的工作包括混合检索与 query router、Memory-Aware 查询改写、ACL 权限过滤、Citation Pipeline 和 frozen + rolling 双轨评测体系。

除此之外，我还做过乳腺癌良恶性预测系统，完整实践了机器学习模型训练、评估和 Streamlit 部署；也在 convoloo 实习期间做过基于 Stable Diffusion、ControlNet 和 LoRA 的故事插画生成系统。我的优势是 AI 应用工程化落地能力，能够把模型、检索、后端、前端、异步任务、部署和质量评测整合成真正可用的系统。
