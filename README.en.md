...
`boss schema` currently exposes 36 top-level commands, plus 9 first-level recruiter subcommands under `hr`, grouped below by workflow:
...
| **Boss RAG** | `rag init` · `rag import-messages` · `rag ingest-mock` · `rag sync-jobs` · `rag sync-messages` · `rag draft` · `rag review` · `rag approve` · `rag audit` |
...
- `approve != send`; V1 does not ship a real send path in the MVP acceptance flow
