...
TOOLS = [
	...
	Tool(
		...
		inputSchema={
			"type": "object",
			"properties": {
				"file": {"type": "string", "description": "待导入文件路径"},
				"format": {
					"type": "string",
					"description": "导入格式",
					"enum": ["json", "md", "csv"],
	...
	Tool(
		...
		inputSchema={
			"type": "object",
			"properties": {
				"file": {"type": "string", "description": "mock envelope 文件路径"},
			},
			"required": ["file"],
		},
	),
	Tool(
		...
		inputSchema={
			"type": "object",
			"properties": {
				"query": {"type": "string", "description": "可选搜索关键词；不传时按当前本地上下文同步"},
			},
			"required": [],
		},
	),
	Tool(
		...
		inputSchema={
			"type": "object",
			"properties": {
				"conversation_id": {"type": "string", "description": "可选会话 ID；不传则按默认范围同步"},
			},
			"required": [],
		},
	),
	Tool(
		...
		inputSchema={
			"type": "object",
			"properties": {
				"conversation_id": {"type": "string", "description": "按 conversation_id 生成该会话草稿"},
				"message_id": {"type": "string", "description": "按 message_id 生成单条消息草稿"},
			},
			"required": [],
		},
	),
	...
	Tool(
		...
		inputSchema={
			...
			"properties": {
				...
				"copy": {"type": "boolean", "description": "是否复制草稿到剪贴板", "default": False},
			},
			"required": ["draft_id"],
		},
	),
	...
	Tool(
		...
		inputSchema={
			"type": "object",
			"properties": {
				"draft_id": {"type": "string", "description": "可选草稿 ID；不传则查看全部 audit 日志"},
			},
			"required": [],
		},
	),
...
def _run_boss(*args: str) -> dict[str, Any]:
	...
	boss_executable = shutil.which("boss")
	if boss_executable:
		cmd = [boss_executable, "--json", *args]
	else:
		cmd = [sys.executable, "-c", "from boss_agent_cli.main import cli; cli()", "--json", *args]
