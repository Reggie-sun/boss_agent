...
from boss_agent_cli.config import DEFAULTS, config_env_sources
...
def config_list_cmd(ctx: click.Context) -> None:
	...
	env_sources = config_env_sources()
	...
	for key in sorted(_PUBLIC_CONFIG_KEYS):
		...
		items.append({
			...
			"value": current_val,
			"default": default_val,
			"source": f"环境变量:{env_name}" if env_name else ("用户配置" if is_custom else "默认值"),
		})
